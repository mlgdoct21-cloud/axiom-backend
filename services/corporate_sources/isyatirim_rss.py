"""İş Yatırım araştırma WordPress RSS adaptörü — Kurumsal Sentez Faz S1a.

`mahfi_rss.py` forkudur (o da fed_rss forku): saf parser (`parse_feed`)
network fetch'ten (`fetch_feed`) ayrı; ETag/If-Modified-Since persist'i
`corporate_source_state` tablosunda (source='isyatirim').

Mahfi'den farklar:
- WordPress RSS → `content:encoded` FULL-TEXT taşıyor (truncated=False
  beklenir; Mahfi alt=rss'in tersine kesik değil).
- `dc:creator` → yazar var; `IsYatirimPost.author` (attribution chip için
  Commit 2'de kullanılacak — telif kuralı: tez+atıf+link).
- 0-entry fallback = kategori feed → ana `/feed/` (Atom yok).

S1a kapsamı: çek + parse + pencere + idempotent haftalık event_id.
Gemini/broadcast/scheduler YOK. Fail policy: sessiz skip + log.
NOT: İş Yatırım günde çok rapor tipi yayınlar (Piyasalarda Bugün, ELÜS
Bülteni, Pay Geri Alımları, Yabancı Oranları...) — tip filtresi Commit 2'nin
işi; bu adaptör hepsini döndürür.
"""
...
logger = get_logger("corporate.isyatirim_rss")

FEED_URL = "https://arastirma.isyatirim.com.tr/category/gunluk-raporlar/feed/"
FALLBACK_URL = "https://arastirma.isyatirim.com.tr/feed/"

SOURCE_KEY = "isyatirim"
...
class IsYatirimPost:
    ...
    author: str = ""             # dc:creator (attribution chip — Commit 2)


def _strip_html(raw: str) -> str:
    """HTML → düz metin. tag strip → entity decode → akıllı tırnak →
    whitespace collapse. BS4 KULLANILMAZ (fed_statement/mahfi pattern)."""
    if not raw:
        return ""
...
def parse_feed(xml_bytes: bytes) -> list[IsYatirimPost]:
    """Saf parser — feed byte'ları → IsYatirimPost listesi. Network YOK.
...
async def fetch_feed(
    *,
    ...
    """Kategori feed'ini çek. ETag / If-Modified-Since aware.
    ...
    Kategori feed 0 entry verirse ana `/feed/` bir kez daha denenir.
    """
    headers = {
    ...
    async def _get(url: str) -> tuple[Optional[bytes], dict]:
        try:
            ...
            logger.warning(f"isyatirim_rss fetch failed ({url}): {e}")
            return None, {"status": 0, "error": str(e)}

        if resp.status_code == 304:
            return None, {"status": 304}
        if resp.status_code >= 400:
            logger.warning(f"isyatirim_rss HTTP {resp.status_code} ({url})")
    ...
    try:
        if not (feedparser.parse(body).entries or []):
            logger.info("isyatirim_rss: kategori feed 0 entry — ana /feed/ deneniyor")
            alt_body, alt_meta = await _get(FALLBACK_URL)
            if alt_body is not None:
                alt_meta["fallback"] = "main_feed"
                return alt_body, alt_meta
    except Exception as e:  # noqa: BLE001 — fail policy: skip + log
        logger.warning(f"isyatirim_rss entry-count probe failed (yok sayılıyor): {e}")

    return body, meta


def posts_in_window(
    posts: list[IsYatirimPost], start: datetime, end: datetime
) -> list[IsYatirimPost]:
...
def week_event_id(monday_utc: datetime) -> str:
    """Haftalık idempotency anahtarı — sha1('isyatirim-week|<iso>')[:16].

    Mahfi'den farklı prefix → kaynaklar arası event_id çakışmaz
    (corporate_syntheses UNIQUE(event_id,tier))."""
    seed = f"isyatirim-week|{monday_utc.date().isoformat()}"
